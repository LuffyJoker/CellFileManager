            ...
            if (!TextUtils.isEmpty(childPath)) {
                ...
                if (file.exists()) {
                    ...
                    if (category == FileCategory.Folder.ordinal) { //打开文件夹
                        ...
                        if (file.isFile) {
                            if (childPath!!.toLowerCase()
                                    .endsWith(".zip") || childPath.toLowerCase().endsWith(
                                    ".rar"
                                )
                            ) {
                                // todo 打开压缩文件
    ...
    override fun initMenus(): XgimiMenuItem {
        ...
        menu.addMenu(
            XgimiMenuItem(
                ...
                if (OperationConfigure.getSortOrder() === Comparators.SortMode.NAME.ordinal) {
                    getString(R.string.time_sort)
                } else {
